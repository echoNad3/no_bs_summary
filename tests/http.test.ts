...
describe('fetchWithOneRetry', () => {
  it('does not retry on success', async () => {
    ...
    const outcome = await fetchWithOneRetry('https://x.test', {}, ctx());
  ...
  it('retries once after a 500', async () => {
    ...
    vi.stubGlobal('fetch', mock);
    const context = ctx();
    const outcome = await fetchWithOneRetry('https://x.test', {}, context);
    expect(outcome.response.status).toBe(200);
  ...
  it('skips the retry when Retry-After does not fit the deadline', async () => {
    ...
    vi.stubGlobal('fetch', mock);
    const outcome = await fetchWithOneRetry('https://x.test', {}, ctx(500));
  ...
  it('keeps the original error message when the retry also fails', async () => {
    ...
    vi.stubGlobal('fetch', mock);
    await expect(fetchWithOneRetry('https://x.test', {}, ctx())).rejects.toThrow('first failure');
  });
  ...
  it('does not retry when the deadline aborts the request', async () => {
    ...
    vi.stubGlobal('fetch', mock);
    await expect(fetchWithOneRetry('https://x.test', {}, ctx())).rejects.toThrow('aborted');
    expect(mock).toHaveBeenCalledTimes(1);
  });
});
